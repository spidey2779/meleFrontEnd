...
document.getElementById('AdminSheetBtn').addEventListener('click', function() {
  ...
  function generatePDF() {
    ...
    const uniqueRoomNumbers = [...new Set(mydata.flatMap(roomData => Object.keys(roomData)))];
    ...
    uniqueRoomNumbers.forEach(roomNumber => {
      ...
      for (let row = 0; row < 6; row++) {
        for (let col = 0; col < 4; col++) {
          ...
          pdf.text(x + boxWidth / 1.3 + 3, y + boxHeight / 4 - 3, seatNumber.toString(), {
            align: 'center',
            baseline: 'middle',
            angle: 0,
          });
          ...
          const matchingDetails = detailsSet.filter(detail => detail.seatNo === seatNumber);
  ...
  document.getElementById('InvizilatorSheetsBtn').addEventListener('click', function (e) {
    generatePDF();
  });
